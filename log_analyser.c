...
char* ParseString(char** time, char** request, char** status) {
    ...
    *time = strtok(NULL, "]", &buffer);
    strtok(NULL, "\"", &buffer);
    *request = strtok(NULL, "\"", &buffer);
    *status = strtok(NULL, " ", &buffer);
...
int ParseTime(char* timeOfRequest) {
    ...
    currString = strtok(timeOfRequest, "/", &buffer);
    time.tm_mday = atoi(currString);
    currString = strtok(NULL, "/", &buffer);
...
int Period() {
    ...
    char* buffer;
    char* currString;
    currString = strtok(periodOfTime, " ", &buffer);
